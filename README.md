...
| `alc_pkg_doctor` | Diagnose package state (read-only). Returns JSON with seven buckets: `healthy` / `incomplete_pkg` / `installed_missing` / `symlink_dangling` / `path_missing` / `missing_meta` (installed pkg whose `init.lua` lacks `M.meta.name`) / `missing_hub_index` (Collection-mode project root with 2+ pkg dirs but no `hub_index.json`). Safe to invoke freely — no filesystem writes |
