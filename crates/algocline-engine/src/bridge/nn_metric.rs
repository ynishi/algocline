//! `alc.nn.metric.*` — distribution distance / entropy primitives, a
//! cluster bootstrap, plus a per-VM Lua-side `registry` for name → fn
//! lookup (feature `nn`).
//!
//! # Layer boundary
//!
...
/// / `entropy`), then [`bootstrap_ci_impl`], then the pure-Lua
/// `registry` sub-table.
///
