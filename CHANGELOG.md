...
- **`ALC_HOME` now moves everything the `alc` binary itself resolves.** The server already read its app root from `$ALC_HOME`, but three paths in the binary were built from the home directory directly: the type stubs every startup writes (`alc.d.lua` / `alc_shapes.d.lua`), the packages directory and config template `alc init` writes, and the `packages/` entry in the Lua search path of the server and of each pool worker. So an `alc` started with `ALC_HOME` set still rewrote `~/.algocline/types/` on every start, served type stubs from a directory it never wrote to, and could `require` packages from `~/.algocline/packages/` that the isolated root did not have. All of them now go through `AppConfig::resolve_app_dir` (now public), the one resolution the server uses. Without `ALC_HOME` nothing moves.
...
- **A decode step forwards one token instead of the whole history.** `arch/kv.rs` adds `KvCache` and `forward_with_cache` on `Gpt2Model` / `TinyLlamaModel` (plus the conditioned / weighted / allowed-id variants on GPT-2); `alc.nn` sessions over a trainable architecture hold one cache each. Generating `n` tokens now runs the model over `n` positions rather than `1 + 2 + … + n` — every step used to recompute each earlier position's keys and values from weights that had not changed. What is cached is the post-rotation, pre-broadcast K/V: post-rotation because RoPE is a function of the absolute position and rotating twice would be wrong, pre-broadcast because storing grouped-query repeats would hold `heads / kv_heads` copies for no gain. The Lua surface is unchanged. Fenced by equivalence rather than by shape: a cached decode is asserted to produce the logits a full re-forward produces, at every step, across all four position schemes (learned / RoPE / ALiBi / none), with a conditioning table attached, on both architectures, batched, and once more at the session level. A cache belongs to one sequence and refuses a forward at another batch size or from a model with a different layer count; the context window is now counted against what the cache holds, or a decode loop would walk past `ctx` one token at a time. (#13)
- **`alc.nn.sampler.penalized(sampler, opts?)` — repetition, frequency and presence penalties.** Every sampler read one logits row and nothing else, so none could tell a token the model had emitted six times from one it had never emitted. The three are not variants of one another: repetition is CTRL's multiplicative factor (dividing a positive logit and multiplying a negative one — one rule for both signs would *raise* the negative logit and reward the repeat), frequency subtracts once per occurrence, presence subtracts once for any occurrence at all. `opts.window` bounds what is counted, because a long generation otherwise accumulates a penalty against every word it has used. The prompt is counted only if the caller says so (`opts.history`): penalising it discourages a summary from reusing the words it was given, which is sometimes exactly wrong. `sampler:observe(id)` feeds in a token the sampler did not produce — refused where it would reach only half the state, since a penalised-over-constrained sampler keeps a grammar prefix this verb cannot advance and the mask one token later would be the mask for a shorter sequence. The wrapper consumes the inner handle as `constrained` does, keeps `is_done` reachable when it wraps a constrained sampler, and refuses to wrap twice. (#13)
- **`handle:embed(tokens, opts?)` — a model trained here can be used as an encoder.** The generation surface exposed logits and nothing else, and the hidden state is not recoverable from them (the head projects `dim` onto a vocabulary and that projection is not invertible). `Gpt2Model::hidden` / `TinyLlamaModel::hidden` return the `[batch, seq, dim]` state after the final norm — the existing forward with its last step split off, so the two cannot drift, asserted by applying the head to `hidden` and reproducing `forward`. `pooling` turns it into one vector: mean (the default), last (what a decoder-only objective actually builds), or max, with `lengths` to keep padding out of the answer. Lua returns a flat array of numbers, since an embedding is consumed by a distance or a store rather than by a handle this crate defines. The llama adapter refuses the call by name — its forward returns the head's output and there is no hidden state behind it, and a model built with a conditioning table refuses an unconditioned call the same way its generation path does, rather than answering with an embedding of a model the caller did not build. (#13)
- **`opts.metrics_every` — a run leaves a curve.** One JSON object per N steps appended to `<card_id>-metrics.jsonl` beside the checkpoints: step, loss, lr, and `grad_norm` / `val_loss` where the run has them. What a run produced reached a record only as final values and as `Candidate` rows at keep time, and converged / stalled / diverged / still-descending are four different runs with the same final loss. JSON Lines, appended as the run goes, so an interrupted run leaves a file valid up to its last complete line. Off by default. (#13)
- **`opts.early_stop = { patience, min_delta }` — stop a run that stopped improving.** Watches the held-out loss and nothing else: stopping on the training loss would stop when the model stopped fitting the data it is being fitted to. Both keys are required with no default on either — `patience` alone stops on noise, `min_delta` alone never stops a run creeping down by nothing. A rule set without `eval_every` and a validation dataset is refused rather than left never to fire, and a non-finite loss counts against patience, so a run that diverges into NaN stops on the same rule instead of running to the end producing nothing. The stopping step is still a step the hook sees: the `ckpt_every` block runs before the exit, so a selection hook is shown the step the run ends on — the one it most wants to keep. A stopped run records `metrics.early_stop = 1`. (#13)
- **`Sampler::sample_batch`** takes `[batch, vocab]` and returns one id per row — the loop the caller previously had to write, with the same per-row RNG sequence. The two stateful wrappers override it to refuse: one prefix and one history cannot describe several sequences, and folding every row's tokens into one state would return valid ids constrained against a sequence none of the rows is. The refusal is delegated through `Box<dyn Sampler>` so erasure cannot re-enable the mixing. `GenSession` remains one sequence per session. (#13)
- **`handle:export_gguf(path, opts?)` — the weights in the format llama.cpp, Ollama and LM Studio read**, quantized on the way out (`f32` / `f16` through the `Q*_0` / `Q*_1` families to the K-quants). Everything trained here landed in safetensors, which the training side reads and nothing else does, so a model trained here could be evaluated here and nowhere else. The tensors are renamed to the GGUF convention, the architecture's key-value metadata is written, and an HF `tokenizer.json` is embedded when one is given. Nothing transposes: ggml stores a matmul weight input-dimension-first, candle's `Linear` holds `[out, in]`, and candle's GGUF writer reverses dimensions, so the tensors pass through unchanged. Fenced by tests: the file reads back, every tensor survives with its GGUF name and shape, the metadata says what it was asked to, `Q8_0` stays inside its block format's error, and the norms stay full precision the way llama.cpp keeps them. **Not fenced: that llama.cpp loads it** — that takes llama.cpp, which this repository neither builds nor vendors, and the remaining step is one `llama-cli -m <file>` elsewhere. A tied head is written out as `output.weight` rather than left to the reader to infer: GGUF has a slot for it and llama.cpp reads that slot, so a file without one is a file whose output projection is missing. `<arch>.vocab_size` is written beside the other architecture keys for the same reason. Refused rather than approximated: a parameter the naming map has no slot for, a width a block format cannot take (with the requirement stated), a custom architecture whose shape GGUF has no key set for, a `pretrained = true` handle whose tensors live behind an unnamed mmap, the inference-only adapter, and a vocabulary with a gap in its ids. (#13)
- **`alc.nn.card.export(card_id, out_dir, opts?)` — a model trained here leaves algocline in the vocabulary the rest of the ecosystem reads.** A Card lives in a store that does not travel with the weights, and each outside reader looks in its own place: the Hugging Face Hub reads a model card's YAML front matter, GGUF runtimes read `general.*`, a safetensors reader has `__metadata__`. The export writes `README.md` (front matter + a body with everything the front matter has no key for), `config.json` (the shape, for algocline's loader), `model.safetensors` (the weights with `__metadata__`) and, when `opts.gguf = { precision, tokenizer }` is given, `model.gguf`. The Card is resolved the way `load_handle` resolves it, `bundle_ref` and channel checks included, and then loaded before anything is written — so the export needs the Card to be loadable on this host (its recorded device and dtype), exactly as `load_handle` does, and a Card whose bundle does not fit the shape it names is refused. One mapping feeds every file: `training_path` `full_ft` / `lora` / `merged` → `base_model_relation` `finetune` / `adapter` / `merge`; a `lineage.parent` that is a Hub repo id (exactly `org/name`, at most 96 characters, each segment in `[A-Za-z0-9._-]` with no leading or trailing `-` / `.` and no `--` / `..`, and not algocline's own `cards/…` / `nn/…` reference forms) → `base_model` and `general.base_model.*`; a Hub `lineage.training_data` → `datasets` and `general.dataset.*`; `opts.license` → `license` and `general.license`; the Card's name → `general.name`, which was the preset variant; `tags: [algocline, candle]` → `general.tags`. The safetensors header carries `format = "pt"` and everything else under `alc.` (`alc.schema = "1"` and `alc.kind = "export"`, which together name the key set, `alc.card_id`, `alc.architecture`, `alc.training_path`, `alc.lineage.*`, `alc.hyperparams`, `alc.metrics`, `alc.config`), plus `alc.tensor_sha256`: the lowercase hex SHA-256 of the data section — ModelSpec's `hash_sha256` range without its `0x`, and not the Hub's whole-file LFS `sha256`, which the README says. The weights are not re-serialised: a new header goes in front of the same data section, written in a fixed order (metadata keys sorted, tensors by offset), so two exports of one Card give byte-identical files, and the digest is taken while the data is copied, so it is of the bytes that were written. Not written: `library_name`, `pipeline_tag`, `model-index`, `general.uuid`, and `model_type` / `architectures` in `config.json` — the export does not claim `transformers` compatibility. Refused: a LoRA Card, whose bundle is a delta (`merge_lora` it first); a custom-architecture Card without `candle.custom` and custom+MoE, with the loader's own messages (the shape rules now live in `NnCardMeta::gpt2_config` / `tinyllama_config`, shared by both); a license with a control character; a file already in `out_dir`; and an option it does not read. No file is ever replaced: the weights and the GGUF are written to a temporary file and hard-linked into place (`export::publish_new`), which fails rather than overwrites if a file appeared meanwhile, and `config.json` / `README.md` are created with `create_new` — so `out_dir` has to be on a filesystem with hard links. If a later step fails, the files the call published are removed. `gguf::export_gguf_with_metadata` is the new GGUF entry point underneath: an extra key replaces a same-named one rather than being written twice, and `export_gguf` delegates to it. **Known limitations:** `distillation` has no `base_model_relation` value, so none is written, and when it is absent the Hub infers one — a distilled model may be shown as a fine-tune, and nothing in the file can prevent that. A lineage value that is a local relative path of the form `a/b` (`data/train.jsonl`) cannot be told apart from a Hub repo id and is written as one. (#14)
- **`handle:beam_search(prompt, opts?)`** — `beams`, `max_new`, `length_penalty`, `eos`, returning beams best first. Every sampler commits to one token and never reconsiders, so a high-probability continuation reachable only through a mediocre first token was unreachable. Scores are sums of log-probabilities, because the product of a few hundred probabilities underflows f32 and every beam would score zero; `length_penalty` divides by `length^α` (Wu et al. 2016 §7), without which every extra token lowers the score and the shortest beam always wins. `BeamModel` is a trait the caller implements, since how a sequence becomes log-probabilities is theirs. It is a different objective from sampling, not a better sampler — the most likely sequence, which is what a translation or a short structured answer wants and not what open text wants. (#13)
- **`FullFtConfig::grad_checkpoint` (Lua: same key) — recompute activations instead of keeping them.** The forward kept every intermediate of every block alive until the backward read it, so context length and batch size were bounded by activation memory rather than by the parameters. This keeps one tensor per block and pays a second forward pass (Chen et al. 2016). The gradients are the same gradients, asserted parameter by parameter against an ordinary backward and again at the run level — four steps with and without, from one seeded initialisation, land on the same weights. `arch::blockwise::Checkpointable` is the view the loop needs; every method refuses by default, so a model with no blockwise view writes one empty impl. Refused where it cannot be honoured: the conditioned and allowed-id entry points, a mixture-of-experts block whose load-balancing term is part of the loss, and a model reading an input channel the blockwise surface does not take. (#13)
- **`BundleIdentity` — a checkpoint now says what it is.** Architecture, vocabulary, context window, dtype, step and the Card that wrote it, in the safetensors `__metadata__` header and in a `<stem>.json` sidecar, both serialised from one value so they cannot disagree. The format states outright that this belongs in a header the writer supplies and candle's `VarMap::save` supplies none, so everything identifying a bundle lived in its Card — in a store that does not travel with the file. The header uses the vocabulary `alc.nn.card.export` writes: `format = "pt"`, the one key safetensors readers share and the value every PyTorch-convention writer puts there, and everything of ours under an `alc.` prefix — `alc.schema = "1"`, `alc.kind = "checkpoint"`, `alc.producer = "algocline <version>"`, `alc.architecture`, `alc.vocab`, `alc.ctx`, `alc.dtype`, `alc.step`, `alc.card_id`. An algocline bundle is recognised by `alc.schema`, not by `format`; `alc.schema` and `alc.kind` together say which key set follows (`"export"` for a file `alc.nn.card.export` wrote). Written by all three training surfaces — `run_full_ft`, `run_distill`, and `run_lora_ft`, whose adapter records the base it loads onto, because that is the model a reader has to rebuild before the delta means anything. `read_bundle_header` reads it back, `None` for a file with no header. The tensor data section of two saves of one map is byte-identical; the header is not, because `safetensors` 0.8 serialises `__metadata__` from a `HashMap` — a byte-stable file is what `alc.nn.card.export` writes. Hyperparameters are deliberately not in it: they describe the run, the Card records them, and a reader loading these weights does not need the learning rate. (#13)
- **`TokenBitset` and the `accept` / `rollback` / `reset` verbs on `Constraint`.** XGrammar and llguidance arrived at the same shape without a specification saying so — a 32-bit-word bitset with a set bit meaning allowed, and compute-mask → accept → rollback → reset. `ConstrainedSampler` keeps one bitset and fills it each step, so a decode loop allocates nothing per token, and `RegexConstraint` keeps its DFA state across `accept`, which is the O(n) → O(1) per token (it re-walked the whole generation once per token before, O(n²) bytes over a generation). The trait's new methods have default bodies, so every existing implementation works unchanged. Asserted rather than assumed: the kept state answers what the prefix walk answers, at every position. (#13)
- **`opts.seed` on `alc.nn.preset.gpt2` / `.tinyllama` and on the dataset opts.** Two runs of one config differed in two places and neither could be pinned. Initialisation is seeded through a `VarBuilder` whose backing store draws the values itself, honouring the `Init` hint each parameter was declared with — the architectures are untouched, and a walker that re-drew parameters after construction would be a second copy of every architecture's initialisation, free to drift. Not `Device::set_seed`: candle has one and it works on CUDA and Metal, but the CPU backend answers `cannot seed the CPU rng with set_seed`, and a run whose reproducibility came from the backend would be repeatable on the GPU and not on the laptop. A seed on a pretrained handle is refused — nothing is drawn there. What stays outside a seed's reach (GPU reduction order, cuDNN algorithm selection, sampling inside a forward pass) is written down in `arch/seeded.rs` rather than implied away. (#13)
- **Cards can now live in a [cardbox](https://github.com/ynishi/cardbox) store instead of in files.** `CardboxStore` is a second `CardBackend`, selected with `[setting.card].backend = "cardbox"` and rooted at `[setting.card].cardbox_root` (default `~/.algocline/cardbox`); `alc info` reports the resolved backend and its root under `card_backend`. It drives the `cardbox` CLI rather than linking the crate — `runcard` pulls `htl` → mlua 0.12 while this workspace is on mlua 0.11, and both claim `links = "lua"`, so cargo refuses a graph containing the two. It is the first backend to implement the Card lifecycle: a cardbox Card is opened and closed, which is what `FileCardStore` has no interval for. The write mapping was measured against a real store of 1104 Cards and is the load-bearing part — a Card's `metadata` goes into the close's `stats.metadata`, because that is where cardbox's `compat find` translates the v0 path `metadata.nn.architecture` to, and metadata written into `params` instead makes the v0 predicate `{metadata:{nn:{architecture:…}}}` return **0 rows** rather than fail. Likewise `created_at` is a tag and not `opened_ms` (run data versus the log's event time — they are different quantities, and `compat find` refuses `order_by created_at` by name), `metadata.prior_card_id` is `open --parent`, and `metadata.prior_relation` — which has no slot — is the tag `lineage.relation`. Queries are handed to `cardbox compat find`, which owns the v0 `where` translation, rather than reimplementing it here. **`skipped` has no flag in cardbox**, whose only states are `open` / `closed_ok` / `closed_failed`: every close writes the v0 status to a `run.status` tag for all three statuses uniformly and `skipped` closes `--ok`, since a skipped run did not fail and leaving its Card open would strand it — `get` reads the tag in preference to the state, so `close(skipped)` round-trips as `skipped` and a `where` on `run.status` still tells the three apart. Where the backend cannot be faithful it refuses rather than approximates: a top-level section with no slot (`strategy_params`), an `append` of anything but `review` / `caveats` (those become a human eval), a second `order_by` key, and `_or` / `_not` over Cards all error with a message naming what the alternative would have cost. **The backend needs cardbox 0.1.2 or newer**: from 0.1.2 a `find` / `list` / `compat find` row answers with the Card's current `tags`, which is what lets a listing row report `created_at` and `run.flow` at all — both are tags, and before 0.1.2 a row carried none. There is no version gate, because on an older cardbox the writes and `get` are unaffected and a row simply reports both as unknown (still without standing `opened_ms` in for a `created_at` it is not), so the degradation is a thinner projection rather than a dead backend. One gap remains, stated rather than papered over: an appended `review` cannot be read back, because cardbox 0.1.2 still has no read verb returning eval payloads — `get` surfaces only the count, and the payload is reachable only in the whole-store `export` event log. (#12)
- **A Card can now have a lifecycle: `open` at the start of a run, `close` either way.** `CardBackend` gains `open(input) -> (card_id, path)` and `close(card_id, CloseOutcome) -> card`, exposed as `alc.card.open` / `alc.card.close` in Lua, `EngineApi::card_open` / `card_close`, and the MCP tools `alc_card_open` / `alc_card_close`. `CloseOutcome` is `{ status, stats?, cost?, error? }` where `status` is one of `succeeded` / `failed` / `skipped` — a failed run closes its Card, it does not leave it open. Both the trait methods and the `EngineApi` methods have default bodies that return an error, so the lifecycle is an opt-in backend capability and no existing implementor breaks. **`FileCardStore` deliberately does not implement them**: a file Card is written once, complete, by `create`, so there is no interval during which an open Card exists — on the default backend both verbs return an error naming `create` as the one-shot alternative. The pair is gated as one by `[setting.card].run`: with the gate off, `open` and `close` both short-circuit with `nil` before the store is touched (gating only `close` would strand opened Cards), while outcome validation runs before the gate so an invalid status is an error either way. (#12)
- **A Card can now record what ran, so `model.id` stops being used for it.** `[run].flow` names the orchestrator / driver / pipeline that produced the Card (`code_review`, `nightly_eval`), leaving `model.id` for an actual model identifier (`claude-opus-4-6`, `cyankiwi/Qwen3.8-27B-AWQ-INT4`). The two were not being kept apart: across one real store 426 of 610 Cards carried a flow name in `model.id` and only 184 carried a model id, which makes a query for "everything from this model" wrong by roughly 70%. The cause was structural rather than careless — `model.id` was the only field that reached a Card's name (`card_id` is minted as `{pkg}_{short name}_{timestamp}_{hash}`), so a strategy that wanted its orchestrator visible in the id had exactly one field to put it in. **`run.flow` now feeds that name segment in `model.id`'s stead whenever it is present**, which relieves the pressure that drove the misuse; a Card that sets no `flow` mints exactly the id it did before. `flow` is a sortable and filterable dotted path like any other (`where = { run = { flow = "code_review" } }`, `order_by = "run.flow"`), and `alc.card.list` / `alc.card.find` rows carry it as `flow` beside `model`. It is deliberately not name-validated — the one place it reaches a name is that `card_id` segment, which keeps ASCII alphanumerics only, exactly as it already does for a model id containing `/`. `CloseOutcome` gains nothing: what is running is known when a run starts, so `flow` travels in `alc.card.open`'s input. (#12)
...
- **Padding is no longer scored in the training loss** — `DatasetOpts::mask_pad`, on by default (Lua: `opts.mask_pad`). `Loss::compute` has taken a `loss_mask` since distillation landed and nothing but `TeacherCardDataset` ever fed it, so every position a short row was padded with was a target the run spent gradient on, and the reported loss sat below the model's real next-token loss by whatever share of the batch was filler. The mask is derived from the row length rather than from comparing tokens against `pad_id`: a row may hold the pad id as content — at the default `pad_id = 0` it is GPT-2's `<|endoftext|>` — and a token-equality test would stop training the model on its own end token. A row that should teach where it ends carries that token itself. Batches whose rows all reach `ctx_len` still carry no mask, so a **packed corpus is bit-identical**; a corpus with short rows will report a different (higher, and correct) loss than it did before. A batch whose mask scores nothing at all is refused rather than run: the masked mean divides by `max(mask_sum, 1)` to avoid `NaN`, so such a batch answers `0.0` — a step with no gradient, reported as the best loss the run has seen and latched there for the rest of it. `opts.mask_pad = false` reproduces the old numbers. (#13)
- **`DatasetOpts::shuffle` shuffles.** It reversed the rows — a fixed permutation, so a corpus grouped by source came out grouped by source, backwards. It is now a real Fisher–Yates draw under `DatasetOpts::seed`. Every dataset built with `shuffle = true` before this got the reversal. (#13)
- **The constraint mask no longer round-trips the logits to the host.** Masking read the logits back with `to_vec1`, scattered `-inf` into the host copy, and uploaded the result — on an accelerator a device→host→device trip of the full vocabulary at every decoded token, against a model whose forward never left the device. The mask is now an additive bias applied on the device, and the last mask's tensor is kept, so a constraint whose mask does not change between steps uploads it once per generation rather than once per token (every allow-list and stop-token decode, and a grammar between parse-state changes). One value changes its bits: a logit of `-0.0` comes back `+0.0`, because IEEE-754 rounds a sum of unlike zeros that way — equal under every comparison the samplers make, recorded rather than rounded over. (#13)
- **F32 AdamW no longer routes through `candle_nn::AdamW`.** That type keeps its moments in private fields with no accessor, so a run using it could never write its optimizer state — for the default optimizer at the default dtype, which is nearly every run. `MixedAdamW` implements the same update term for term and its FP32-on-FP32 case is pinned against the stock one to within `1e-6` by an existing parity test; F32 runs may differ from previous ones at that scale. (#13)
...
- **Checkpoint rotation no longer counts an optimizer-state sidecar as a checkpoint.** `<prefix>-step<N>.opt.safetensors` shares both the run prefix and the extension, so the listing that sizes the `ckpt_keep` window would have counted them and deleted live checkpoints to make room for the state of checkpoints that had already gone. Found while adding the sidecar; no released version wrote one. (#13)
- `docs/lua-stdlib.md` documented `alc.nn.metric.bootstrap_ci`, which 0.49.0 removed along with the whole `alc.nn.metric` namespace. The section is gone; the replacement is `alc.math.cluster_bootstrap_mean`, as that release's entry says. (#13)
