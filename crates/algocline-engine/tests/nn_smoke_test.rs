...
fn field<'a>(
    out: &'a std::collections::HashMap<String, SmokeValue>,
    name: &str,
) -> &'a SmokeValue {
    out.get(name)
        .unwrap_or_else(|| panic!("field '{name}' missing from smoke output; got keys {:?}", out.keys().collect::<Vec<_>>()))
}

#[test]
fn full_ft_smoke_runs() {
    let out = run_smoke("nn_full_ft_smoke.lua");
    assert!(field(&out, "ok").as_bool(), "full_ft smoke must report ok=true");
...
fn lora_smoke_runs() {
    ...
    assert!(field(&out, "ok").as_bool(), "lora smoke must report ok=true");
...
fn distill_smoke_runs() {
    ...
    assert!(field(&out, "ok").as_bool(), "distill smoke must report ok=true");
