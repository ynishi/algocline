...
- **`BundleIdentity` — a checkpoint now says what it is.** Architecture, vocabulary, context window, dtype and the run that wrote it, in the safetensors `__metadata__` header and in a `<stem>.json` sidecar, both serialised from one value so they cannot disagree. The format states outright that this belongs in a header the writer supplies and candle's `VarMap::save` supplies none, so everything identifying a bundle lived in its Card — in a store that does not travel with the file. Written by all three training surfaces — `run_full_ft`, `run_distill`, and `run_lora_ft`, whose adapter records the base it loads onto, because that is the model a reader has to rebuild before the delta means anything. `read_bundle_header` reads it back, `None` for a file with no header. Hyperparameters are deliberately not in it: they describe the run, the Card records them, and a reader loading these weights does not need the learning rate. (#13)
