...
M.smc_sampled = T.shape({
    ...
    }, { open = true })):describe("All N particles in their final state"),
...
M.quick_voted = T.shape({
    ...
    needs_investigation = T.boolean:describe(
        "True only when outcome == 'truncated' (evidence inconclusive at declared α/β). 'rejected' is a conclusive verdict and does NOT set this flag."
    ),
}, { open = true })
