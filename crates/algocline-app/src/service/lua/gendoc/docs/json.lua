        ...
        elseif c == "\\" then
            ...
            elseif nxt == "u" then
                ...
                if #hex ~= 4 or not hex:match("^%x%x%x%x$") then
                    error(string.format("json: bad \\u escape at byte %d", i), 0)
                end
                local code = tonumber(hex, 16)
                -- basic surrogate pair support (UTF-16 high/low)
                if code >= 0xD800 and code <= 0xDBFF then
                    if s:sub(i + 6, i + 7) ~= "\\u" then
                        error(string.format("json: unpaired high surrogate at byte %d", i), 0)
                    end
                    local lo_hex = s:sub(i + 8, i + 11)
                    if #lo_hex ~= 4 or not lo_hex:match("^%x%x%x%x$") then
                        error(string.format("json: bad low surrogate at byte %d", i + 6), 0)
                    end
                ...
                error(string.format("json: bad escape \\%s at byte %d", nxt, i), 0)
            end
        else
            out[#out + 1] = c
            i = i + 1
...
local function decode_array(s, i)
    ...
    while i <= #s do
        ...
        elseif c == "]" then
            ...
            error(string.format("json: expected ',' or ']' at byte %d, got %q", i, c), 0)
        end
    end
...
local function decode_object(s, i)
    ...
    while i <= #s do
        if s:sub(i, i) ~= '"' then
            error(string.format("json: expected string key at byte %d", i), 0)
        end
        local key
        key, i = decode_string(s, i)
        i = skip_ws(s, i)
        if s:sub(i, i) ~= ":" then
            error(string.format("json: expected ':' after key at byte %d", i), 0)
        end
        i = skip_ws(s, i + 1)
        local val
...
decode_value = function(s, i)
    ...
    elseif c == "-" or (c >= "0" and c <= "9") then
        ...
        error(string.format("json: unexpected char %q at byte %d", c, i), 0)
    end
end
...
function M.decode(s)
    ...
    i = skip_ws(s, i)
    if i <= #s then
        error(string.format("json.decode: trailing garbage at byte %d", i), 0)
    end
    return v
end
