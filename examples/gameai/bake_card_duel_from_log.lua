-- Bake a card duel NPC out of a play log.
--
-- Self-contained script for `alc_run` (`code_file` form). It is the
-- third bake input of the demo: `train_card_duel_npc.lua` learns a
-- hand-written style, `bake_card_duel_persona.lua` learns a style an
-- LLM wrote from one line of prose, and this script learns the moves
-- someone actually played. The log is the `move_log` that
-- `card_duel_interactive` returns from its `end` action, so a human can
-- sit down, play a few hands and hand the session straight to the
-- trainer.
--
...
-- the log the model reproduces, not how the model plays a position the
-- log never contained. Measuring the latter needs a held-out log or a
-- human verdict, and neither is in this script.
--
...
-- `examples/nn_distill_llm_teacher_e2e.lua` uses for its teacher pairs.
-- Replication buys steps, not coverage: a short log trains a model that
-- has seen very few distinct positions, however low the loss goes.
--
-- ctx:
--   moves  -- required; array of logged moves, each
--             `{ round, my_hand, my_points, opp_points, opp_played,
--             action }`. This is the `move_log` array of a finished
--             interactive session, unchanged
--   name   -- required; slug matching ^[a-z0-9_]+$. The Card is pinned
--             to the alias `card_duel_npc_<name>`
...
--             trained. It is accepted so a log bake can be logged
--             next to a style bake with the same run parameters
--
...
-- `loss_descended`, plus the corpus shape. The compliance number is
-- reported, not asserted: what an acceptable fit is depends on the log
-- and on the training budget, and that judgement belongs to the demo
-- runbook.
...
local function log(msg)
    alc.log("info", "[card-duel-bake-log] " .. msg)
end

-- ─── ctx validation ─────────────────────────────────────────────────
--
-- Only the shape of the request is checked here; every field of every
-- move is checked by `card_duel.rows_from_moves`, which names the entry
-- and the reason so a malformed log can be fixed without guessing.

if type(MOVES) ~= "table" or #MOVES == 0 then
    error("bake_card_duel_from_log: ctx.moves is required and must be a non-empty array of moves")
end
if type(NAME) ~= "string" or not NAME:match("^[a-z0-9_]+$") then
    error(
        string.format(
            "bake_card_duel_from_log: ctx.name must match ^[a-z0-9_]+$, got %s",
            tostring(NAME)
        )
    )
end
if STEPS <= 0 or BATCH <= 0 then
    error("bake_card_duel_from_log: ctx.steps and ctx.batch must be positive integers")
end

local ALIAS = "card_duel_npc_" .. NAME
local CARD_NAME = "card-duel-playlog-" .. NAME
...
if VOCAB.size > model_vocab then
    error(
        string.format(
            "bake_card_duel_from_log: alphabet of %d chars exceeds the model vocabulary of %d",
            VOCAB.size,
            model_vocab
        )
    )
...
local base_rows, plays = duel.rows_from_moves(MOVES, {
    ctx_len = ctx_len,
    pad_id = VOCAB.pad_id,
})
...
if type(card_id) ~= "string" or #card_id == 0 then
    error("bake_card_duel_from_log: run_full_ft returned no card_id")
end
...
if type(train_loss) ~= "number" then
    error("bake_card_duel_from_log: metadata.nn.metrics.train_loss missing from the Card")
end
...
if type(appended) ~= "table" then
    error("bake_card_duel_from_log: alc.card.append did not return the merged Card")
end

-- ─── Phase 4: log replay ────────────────────────────────────────────
--
-- Every logged position is decoded through the freshly pinned Card and
-- compared with the rank the human played there. These are the states
...
for i, play in ipairs(plays) do
    local answer = npc.run({
        task = alc.json_encode({ mode = "decide", state = play.state }),
        card_alias = ALIAS,
    }).result
    local rank = tonumber(answer:match("action=(%d+)"))
    if rank == nil then
        error(
            string.format(
                "bake_card_duel_from_log: NPC answer for move %d carried no action: %s",
                i,
                tostring(answer)
            )
        )
    end
    if rank == play.action then
        hits = hits + 1
    end
end
