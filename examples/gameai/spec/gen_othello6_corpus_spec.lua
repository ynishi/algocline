-- spec/gen_othello6_corpus_spec.lua
--
-- Spec for the ctx contract, the sizing and the file contract of
-- `gen_othello6_corpus.lua`.
--
...
--
-- Nothing costly runs. `othello6.build_corpus` is a spy that answers a
-- fixed yield per requested game, so the default of two thousand
-- playouts is read off the request rather than played -- at the measured
-- 0.0156 s per playout, playing them here would cost half a minute per
-- case. One case delegates to the real generator on three games, which
-- is what pins the rows in the file to the width the encoding produces.
--
...
--
--     test_launch(code_file = "examples/gameai/spec/gen_othello6_corpus_spec.lua",
--                 search_paths = { "<repo>/examples/gameai" })
--
-- so `require("gen_othello6_corpus")` / `require("othello6")` /
...
othello.build_corpus = function(policy, opts)
    ...
    -- width, and the default case asks for two thousand of them.
    local rows = {}
...
local function drive(overrides)
    ctx = overrides or {}
    package.loaded["gen_othello6_corpus"] = nil
    return require("gen_othello6_corpus")
end
...
--- A run small enough to read, with the file it writes to.
local function small(overrides)
    local out = { path = CORPUS_FILE, depth = 1, games = 3 }
...
describe("gen_othello6_corpus ctx contract", function()
    it("refuses a run that writes nowhere", function()
        configure()
        local err = drive_error({ depth = 1, games = 3 })

        expect(contains(err, "ctx.path is required")).to.equal(true)
        -- Refused before a game is played: a generation with nowhere to
        -- put its rows costs playouts and leaves nothing behind.
        expect(#CORPUS_CALLS).to.equal(0)
        expect(#PARENT_DIRS).to.equal(0)
    end)

    it("refuses a path that is not a non-empty string", function()
        configure()
        expect(contains(drive_error({ path = 7 }), "ctx.path must be a string")).to.equal(true)

        configure()
        expect(contains(drive_error({ path = "" }), "ctx.path must not be empty")).to.equal(true)
    end)

    it("applies the documented defaults", function()
        configure()
        local out = drive({ path = CORPUS_FILE })

        -- Two thousand playouts is the tens-of-seconds budget at the
        -- measured 0.0156 s per game; the design note's twenty thousand
        -- is five minutes and belongs on a pod.
        expect(out.games).to.equal(2000)
        ...
        expect(CORPUS_CALLS[1].games).to.equal(2000)
    ...
    it("takes the teacher pair and the playout dials from ctx", function()
        configure()
        local out = drive(small({ depth = 4, style = "greedy", seed = 99, random_opening_max = 2 }))

        expect(out.depth).to.equal(4)
        expect(out.style).to.equal("greedy")
    ...
    it("refuses a depth or a style the modules do not carry", function()
        configure()
        expect(
        ...
        expect(
            contains(
                drive_error(small({ style = "aggressive" })),
                "ctx.style = aggressive is not one of corner, mobility, greedy"
            )
        ).to.equal(true)
    end)
    ...
    it("refuses a field of the wrong type instead of coercing it", function()
        configure()
        expect(contains(drive_error(small({ games = "3" })), "ctx.games must be a finite number")).to.equal(
            true
        )

        configure()
        expect(contains(drive_error(small({ games = 2.5 })), "ctx.games must be an integer")).to.equal(
            true
        )

        configure()
        expect(contains(drive_error(small({ games = 0 })), "ctx.games must be at least 1")).to.equal(
            true
        )
    end)
    ...
    it("refuses an unknown ctx field", function()
        ...
        expect(contains(err, "known: ctx_len, depth, games, path")).to.equal(true)
        expect(#CORPUS_CALLS).to.equal(0)
    end)
...
describe("gen_othello6_corpus file contract", function()
    it("writes the meta the bake driver reads back", function()
        configure()
        local out = drive(small({ depth = 4, style = "greedy", games = 3, seed = 99 }))

        local corpus = written_corpus()
        expect(corpus.meta.game).to.equal("othello6")
        expect(corpus.meta.depth).to.equal(4)
        expect(corpus.meta.style).to.equal("greedy")
        ...
        -- reader cannot check itself against.
        expect(key_list(corpus.meta)).to.equal(
            "bos,ctx_len,depth,game,games,random_opening_max,rows_per_game_estimate,seed,style,vocab_size"
        )
        ...
        expect(out.meta.game).to.equal("othello6")
    end)
    ...
    it("writes rows of the width the encoding produces", function()
        configure()
        -- The real generator for this one, on three games: what lands in
        -- the file has to be rows of the width the bake driver will hold
        -- them to.
        CORPUS_REAL = true
    ...
    it("says so when it is replacing a corpus", function()
        ...
        -- condition of, so the replacement appears in the log of the run
        -- that did it.
        local said = false
        ...
        -- And the new corpus is what is there afterwards.
    ...
    it("says so when the generator answers a different count", function()
        ...
        -- disagree is measured under a size it never had, so the break
        -- is reported instead of written.
...
describe("gen_othello6_corpus cost reporting", function()
    it("reports what the generation cost", function()
        ...
        -- multiplies, rather than the figure in the header, which was
    ...
    it("logs the rows, the width and the wall clock", function()
        configure()
        drive(small({ games = 3 }))
