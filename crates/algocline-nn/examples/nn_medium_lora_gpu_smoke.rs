...
//! Δ-only checkpoint save. In addition to the run itself the driver
//! verifies three invariants that matter for "did LoRA actually
//! learn?":
//!
...
//!    below the configured ceiling (default `32 MB`, sized for
//!    GPT-2 medium at rank 16 wrapping the canonical 6 targets =
//!    ~24 MB of parameters plus the safetensors header).
...
//! - `NN_SMOKE_LORA_TARGETS`  (default `full`) — `full` = canonical 6
//!   targets (`q_proj`, `k_proj`, `v_proj`, `o_proj`, `up`, `down`),
//!   `attn` = attention only (`q_proj`, `k_proj`, `v_proj`, `o_proj`).
//! - `NN_SMOKE_DELTA_MAX_BYTES` (default `33554432` = 32 MB) — hard
...
//!   ~700 MB of temp file I/O for GPT-2 medium.
//!
...
/// `full` (canonical 6). Any unknown value falls through to `full`
/// with a warning line — the run should still produce a valid result
/// but the caller sees that the requested preset was not honoured.
fn resolve_targets() -> Vec<String> {
        ...
        .as_str()
    {
        ...
        "full" | "" => LoraConfig::default_targets(),
        other => {
            eprintln!(
                "[smoke] unknown NN_SMOKE_LORA_TARGETS={other:?}; falling back to full 6-target set"
            );
            LoraConfig::default_targets()
        }
    }
}

/// Byte-exact compare of two files, streaming a shared buffer instead
/// of loading both into memory (GPT-2 medium base ≈ 1.4 GB each).
    ...
    if delta_bytes > delta_max_bytes {
        return Err(format!(
            "LoRA Δ bundle size {} B exceeds ceiling {} B (path={:?}). \
             Rank {} × alpha {} × {} targets on GPT-2 medium. \
