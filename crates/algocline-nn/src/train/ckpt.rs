//! Rotating safetensors checkpoint writer, and the verified reader
//! that puts one back.
//!
...
//!
//! Reading lives here too, because the two halves have to agree on what
//! a checkpoint contains. [`restore_into`] loads one into a `VarMap`
//! that is still trainable — which is what separates it from
//! [`crate::arch::Gpt2Model::from_safetensors_file`], whose mmapped
//! `VarBuilder` yields tensors rather than `Var`s and so can only be
//! inferred from.
...
/// restore failed" and "this checkpoint was baked at vocab 3900 and the
/// map wants 4100".
...
pub enum RestoreError {
    ...
    #[error("restore from {path:?}: the VarMap has no registered variables, so a restore would silently do nothing")]
    NoRegisteredVars {
        /// Checkpoint that was being read.
        path: PathBuf,
    },
    ...
    #[error(
        "restore from {path:?}: {} of {registered} registered variables are absent from the checkpoint ({}); \
         training would have continued from their initial values. Use `restore_into_partial` if that is intended",
        .absent.len(),
        preview(.absent),
    )]
    Incomplete {
    ...
    ///
    /// Kept separate from [`RestoreError::Read`] because a caller has
    /// to be able to tell a clean refusal from a half-written map,
    /// which is the distinction the whole verify-then-write structure
    /// exists to preserve.
    #[error(
        "restore from {path:?}: {stage} tensor {name:?} failed while applying the checkpoint; {}: {message}",
...
/// which legs were skipped, otherwise "partial" degrades back into the
/// silence this module is built to avoid.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl RestoreReport {
    ...
    pub fn summary(&self) -> String {
        format!(
            "{} of {} variables restored from {:?} ({} left at their initial value, {} tensors in the file unused)",
            self.restored.len(),
...
/// narrow-purpose choice (a base checkpoint into a LoRA-wrapped map),
/// and a call site that wants it should have to say so by name.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum Scope {
...
/// supported way to start a `run_full_ft` from previously trained
/// weights, whether to resume an interrupted run or to stage a
/// curriculum across several runs.
///
...
/// `path` comes from the caller — at the one call site in this
/// workspace it is a path an operator typed — so the requirement lands
/// there. Checkpoints this crate writes are only created once and
/// unlinked by rotation, which satisfies it for a path that came out of
/// [`CheckpointStore`], but that is a property of those files and not
/// of the argument.
...
fn restore(varmap: &VarMap, path: &Path, scope: Scope) -> Result<RestoreReport, RestoreError> {
    ...
    for (name, var) in registered.iter() {
        ...
        // than a mismatch: there is no `found_dtype` to report. Note
        // that candle can still *read* some of these — its `convert`
        // widens a `U16` tensor into a `U32` one — so the statement is
        // about what a `Var` can be, not about what candle can parse.
...
/// untouched — `MmapedSafetensors::load` builds a tensor and touches no
/// `Var` — and telling that caller to rebuild would send them chasing
...
/// makes the three early returns in `Var::set` unreachable for anything
/// it verified, so a failure that survives to here comes out of the
/// copy itself, and candle does not say whether the destination was
/// left alone. The honest word is "may".
///
...
/// Same truncation for the mismatch list, which needs shapes as well as
