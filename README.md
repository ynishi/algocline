...
| `alc_pkg_list` | List installed packages with metadata. Pass `project_root` to include project-local packages. Each entry includes an optional `warnings` array; packages auto-detected as libraries carry a suggestion to declare `M.meta.type = "library"` explicitly |
...
| `alc_pkg_doctor` | Diagnose package state (read-only). Returns JSON with eleven buckets: `healthy` / `incomplete_pkg` / `installed_missing` / `symlink_dangling` / `path_missing` / `missing_meta` (installed pkg whose `init.lua` lacks `M.meta.name`) / `missing_hub_index` (Collection-mode project root with 2+ pkg dirs but no `hub_index.json`) / `spec_missing` (installed pkg with `spec/` but zero `*_spec.lua` files; opt-in spec discipline check) / `stale_cache` (hub cache file older than 3600s; refresh via `alc_hub_search`) / `unregistered_pkg` (physical dir under `~/.algocline/packages/` with `init.lua` but not registered in `installed.json`, `alc.toml`, or `alc.local.toml`; `suggestion` is an `array<string>` with install / link / remove / source-note options) / `unmarked_library` (package auto-detected as a library — no `M.run` and no `M.meta.type` — with a suggestion to add `M.meta.type = "library"` for explicit declaration; packages with an explicit `M.meta.type` or legacy entries without `type_source` are never flagged). Safe to invoke freely — no filesystem writes |
